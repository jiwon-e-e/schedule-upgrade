...
※ 본 프로젝트는 `REST API 기반 서버`로, 별도의 View 화면은 제공하지 않습니다.
...
| Day 2 | 일정,유저,댓글 CRUD 구현          | 기본 구현                         |
| Day 3 | 업그레이드  | 비밀번호 암호화, 중복 유저 검증, 댓글 개수 조회 |
| Day 4 | 업그레이드               | 페이징, exception ENUM 처리                        |
| Day 5 | 제출               | README 정리,테스트 최종 점검, 리팩토링                        |
